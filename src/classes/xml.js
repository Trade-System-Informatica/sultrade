...
import { URL_SIGISS, URL_SOAPACTION } from '../config';
import { parseString } from 'xml2js';
import { inheritTransformationMetadata } from '@nestjs/mapped-types';


export default class Xml {
    static async formataXml(xml) {
    ...
    static async criaXml(chave) {
        ...
        const json = {
            ...
            aliquota_simples: (info.aliquota_simples != "" && info.aliquota_simples != 0 && info.aliquota_simples != "0.00" ? await Util.formataDinheiroBrasileiro(info.aliquota_simples) : ""),
...
${xml}
        ...
        const envio = await this.formataXml(xml);
        ...
        return ({msg: retornoJson, numero: notaNumero, autenticidade});
    }
} 