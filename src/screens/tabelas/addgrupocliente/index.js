...
class AddGrupoCliente extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (this.state.chave != 0) {
            ...
            if ((e.acessoAcao == "GRUPOS_CLIENTES" && e.permissaoInsere == 0 && this.state.chave == 0) || (e.acessoAcao == "GRUPOS_CLIENTES" && e.permissaoEdita == 0 && this.state.chave != 0)) {
    ...
    salvarGrupo = async (validForm) => {
        ...
        if (parseInt(this.state.chave) === 0 && validForm) {
            await apiEmployee.post(`insertGrupoCliente.php`, {
                ...
                async res => {
                    if (res.data[0].chave) {
                        ...
                        await loader.salvaLogs('os_grupos_clientes', this.state.usuarioLogado.codigo, null, "Inclusão", res.data[0].chave);
            ...
            await apiEmployee.post(`updateGrupo.php`, {
                ...
                async res => {
                    if (res.data === true) {
                        await loader.salvaLogs('os_grupos_clientes', this.state.usuarioLogado.codigo, this.state.dadosIniciais, this.state.dadosFinais, this.state.chave, `GRUPOS DE CLIENTES: ${this.state.descricao}`);
    ...
    openLogs = async () => {
        await this.setState({ logs: await loader.getLogs("os_grupos_clientes", this.state.chave) })
        ...
        return (
            <div className='allContent'>
                ...
                {this.state.redirect &&
                    ...
                    <Header voltarGruposClientes titulo="Grupos de Cliente" chave={this.state.chave != 0 ? this.state.chave : ''} />
                </section>
...
export default connect(mapStateToProps, null)(AddGrupoCliente)
