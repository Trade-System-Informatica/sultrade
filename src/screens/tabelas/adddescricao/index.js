...
class AddDescricao extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (parseInt(id) !== 0) {
            await this.setState({ descricao: this.props.location.state.descricao })
            //console.log('Servicos: ' + JSON.stringify(this.state.tiposervico))
            //await this.loadData(this.state.tiposervico)
            await this.setState({
                descricao: this.state.descricao.descricao,
            })
        ...
        this.state.acessosPermissoes.map((e) => {
            if ((e.acessoAcao == "DESCRICOES_PADRAO" && e.permissaoInsere == 0 && this.state.chave == 0) || (e.acessoAcao == "DESCRICOES_PADRAO" && e.permissaoEdita == 0 && this.state.chave != 0)) {
    ...
    salvarDescricao = async (validForm) => {
        ...
        if (parseInt(this.state.chave) === 0 && validForm) {
            ...
            await apiEmployee.post(`updateDescricoesPadrao.php`, {
                token: true,
                ...
                async res => {
                    ...
                    if (res.data === true) {
                        await loader.salvaLogs('os_descricao_padrao', this.state.usuarioLogado.codigo, this.state.dadosIniciais, this.state.dadosFinais, this.state.chave, `DESCRIÇÃO PADRÃO: ${this.state.descricao}`);
    ...
    openLogs = async () => {
        await this.setState({ logs: await loader.getLogs("os_descricoes_padrao", this.state.chave) })
    ...
    render() {
        ...
        return (
            <div className='allContent'>
                ...
                <section>
                    <Header voltarDescricoesPadrao titulo="Descrições Padrão" chave={this.state.chave != 0 ? this.state.chave : ''}/>
                </section>
                        ...
                        <div className="col-lg-12">
                            <Formik
                                ...
                                onSubmit={async values => {
                                    ...
                                    this.salvarDescricao(validForm)
                                }}
                            >
...
export default connect(mapStateToProps, null)(AddDescricao)
