...
class ContasCorrentes extends Component {
    ...
    componentDidMount = async () => {
        ...
        this.state.acessosPermissoes.map((e) => {
            if (e.acessoAcao == "CONTAS_CORRENTES" && e.permissaoConsulta == 0) {
    ...
    loadAll = async () => {
        await this.setState({
            contas: await loader.getBase('getContasCorrentes.php', this.state.usuarioLogado.empresa),
                ...
                return (
                    <div className='custom-ui text-center'>
                        ...
                        <button
                            ...
                            onClick={
                                async () => {
                                    await apiEmployee.post(`deleteContaCorrente.php`, {
                                        token: true,
                                        chave: chave
                                    }).then(
                                        async response => {
                                            if (response.data == true) {
                                                await loader.salvaLogs('contas_corrente', this.state.usuarioLogado.codigo, null, "Exclusão", chave);
            ...
            <div className='allContent'>
                ...
                <div>
                    ...
                    {!this.state.loading &&
                        <div>
                            <section>
                                <Header voltarFinanceiroTabelas titulo="Contas Correntes" />
                                        ...
                                        <div className="row subtitulosTabela">
                                            ...
                                            <div className="col-2 text-center revertItem" onClick={() => { if (this.state.contas[0]) { this.reverterItens() } }}>
                                                {!this.state.contas[0] && this.state.acessosPermissoes.filter((e) => { if (e.acessoAcao == 'CONTAS_ABERTAS') { return e } }).map((e) => e.permissaoInsere)[0] == 1 &&
                                                    <span className="subtituloships"><Link to={{ pathname: `/financeiro/addcontacorrente/0`, state: { tipo: 1, to: 'contaspagar' } }}><FontAwesomeIcon icon={faPlus} /></Link></span>
                                                }
                                                {this.state.contas[0] &&
                                            ...
                                            <div className="row deleteMargin alignCenter">
                                                ...
                                                <div className="col-2  text-left  mobileajuster4 icones">
                                                    <div className='iconelixo giveMargin' type='button' >
                                                        <Link to=
                                                            {{
                                                                pathname: `/financeiro/addcontacorrente/0`,
                                                                state: { conta: {}, tipo: 1 }
                                                            }}
                                                        >
                                                    ...
                                                    <div className='iconelixo giveMargin' type='button' >
                                                        <Link to=
                                                            {{
                                                                pathname: `/financeiro/addcontacorrente/${feed.Chave}`,
                                                                state: { conta: { ...feed } }
                                                            }}
                                                        >
                                                    ...
                                                    {this.state.acessosPermissoes.filter((e) => { if (e.acessoAcao == 'CONTAS_CORRENTES') { return e } }).map((e) => e.permissaoDeleta)[0] == 1 &&
...
export default connect(mapStateToProps, null)(ContasCorrentes)