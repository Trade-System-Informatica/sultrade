...
class AddPorto extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (parseInt(id) !== 0) {
            await this.setState({ porto: this.props.location.state.porto })
            this.setState({
    ...
    salvarPorto = async (validForm) => {
        ...
        if (parseInt(this.state.chave) === 0 && validForm) {
            ...
            await apiEmployee.post(`insertPorto.php`, {
                token: true,
                values: `'${this.state.sigla.toUpperCase()}', '${this.state.descricao}'`
            }).then(
                async res => {
                    if (res.data[0].Chave) {
    ...
    openLogs = async () => {
        await this.setState({ logs: await loader.getLogs("os_portos", this.state.chave) })
    ...
    render() {
        ...
        validations.push(this.state.sigla && this.state.sigla.length == 3)
        ...
        return (
            <div className='allContent'>
                ...
                {this.state.finalizaOperacao &&
                    <Redirect to={{pathname: '/admin/portos', state:{chave: this.state.chave}}} />
                }

                <section>
                    <Header voltarPortos titulo="Portos" chave={this.state.chave != 0 ? this.state.chave : ''}/>
                </section>
                                        ...
                                        <div className="col-xl-8 col-lg-8 col-md-8 col-sm-8 col-12 ">
                                            ...
                                            <div className="row addservicos">
                                                ...
                                                <div className='col-1 errorMessage'>
                                                    ...
                                                    {this.state.sigla && this.state.sigla.length != 3 &&
...
export default connect(mapStateToProps, null)(AddPorto)
