...
class AddTipoServico extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (parseInt(id) !== 0) {
            await this.setState({ tiposervico: this.props.location.state.tiposervico })
    ...
    salvarServico = async (validForm) => {
        ...
        if (parseInt(this.state.chave) === 0 && validForm) {
            await apiEmployee.post(`insertTipoServico.php`, {
                token: true,
                values: `'${this.state.descricao}', '${this.state.prazo}'`
            }).then(
                async res => {
                    if (res.data[0].chave) {
    ...
    openLogs = async () => {
        await this.setState({ logs: await loader.getLogs("os_tipos_servicos", this.state.chave) })
    ...
    render() {
        ...
        validations.push(this.state.prazo && this.state.prazo > 0)
        ...
        return (
            <div className='allContent'>
                ...
                {this.state.finalizaOperacao &&
                    <Redirect to={{pathname: '/admin/tiposservicos', state: {chave: this.state.chave}}}  />
                }
                                        ...
                                        <div className="col-xl-4 col-lg-8 col-md-8 col-sm-8 col-12 ">
                                            ...
                                            <div className="row addservicos">
                                                ...
                                                <div className='col-1 errorMessage'>
                                                    ...
                                                    {this.state.prazo && this.state.prazo < 1 &&
                                                        <FontAwesomeIcon title='Prazo não pode ser menor que 1' icon={faExclamationTriangle} />
                                                    }
                                                </div>
                                                <div className="col-xl-6 col-lg-6 col-md-6 col-sm-10 col-10 ">
                                                    <input type='number' value={this.state.prazo} onChange={(e) => { this.setState({ prazo: e.currentTarget.value }) }}></input>
                                                </div>
                                            </div>
...
export default connect(mapStateToProps, null)(AddTipoServico)
