        ...
        if ($where != "") {
            ...
            LEFT JOIN os ON os.centro_custo = contas_aberto.centro_custo AND os.centro_custo != "" AND os.centro_custo != 0',
                "GROUP_CONCAT(contas_aberto.Docto SEPARATOR '@.@') AS documento, 
                                          ...
                                          (SELECT (GROUP_CONCAT(os_servicos_itens.valor SEPARATOR '@.@') FROM os AS Ordem LEFT JOIN os_servicos_itens ON Ordem.chave = os_servicos_itens.chave_os WHERE Ordem.chave = os.chave AND os_servicos_itens.cancelada != 1 AND os_servicos_itens.tipo_sub IN (0, 1) AND (os_servicos_itens.repasse = 1 OR os_servicos_itens.Fornecedor_Custeio != '') ORDER BY os.chave DESC GROUP BY os.chave LIMIT 1)) AS valor_eventos,
