...
if ($objData != NULL) {
    ...
    if (strlen($dadosComplementares->{"contribuinte"}) == 11) {
        $numeroIdentificacaoContribuinte = 2;
    } else if (strlen($dadosComplementares->{"contribuinte"}) == 14) {
        $numeroIdentificacaoContribuinte = 1;
    }

    $dataPagamento = date_create($conta->{"Vencimento"});
    $mesAnoCompetenciaGuiaPrevidenciaSocial = date_create($conta->{"mesAnoCompetenciaGuiaPrevidenciaSocial"});
    ...
    $body = [
        ...
        "lancamentos" => [[
            ...
            "codigoReceitaTributo" => $conta->{"codigo_receita"},
            "codigoTipoContribuinte" => $dadosComplementares->{"contribuinte"},
            "numeroIdentificacaoContribuinte" => $numeroIdentificacaoContribuinte,
            "codigoIdentificadorTributo" => $dadosComplementares->{"codigo_identificador_tributo"},
            "numeroReferencia" => $dadosComplementares->{"mes_compet_num_ref"},
            "valorPrincipal" => $dadosComplementares->{"valor"},
            "valorMulta" => $dadosComplementares->{"valor_outros"},
            "valorJuroEncargo" => $dadosComplementares->{"valor_juros"},

            "dataPagamento" => intval(date_format($dataPagamento, 'dmY')),
            "mesAnoCompetenciaGuiaPrevidenciaSocial" => intval(date_format($mesAnoCompetenciaGuiaPrevidenciaSocial, 'dmY'))
        ]]
    ];
    ...
    //$url = "https://api.sandbox.bb.com.br/pagamentos-lote/v1/lotes-gps?gw-dev-app-key=d27bc7790affabc01368e17df0050056b9a1a5bf";
...
$contas->insertTransacao($values, "DARF");
} else {
}
