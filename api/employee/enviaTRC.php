...
if ($objData != NULL) {
    ...
    if (strlen($conta->{"pessoaCPF"}) == 11) {
        $pessoaCPF = intval($conta->{"pessoaCPF"});
        $pessoaCNPJ = "";
    } else if (strlen($conta->{"pessoaCPF"}) == 14) {
        $pessoaCNPJ = intval($conta->{"pessoaCPF"});
        $pessoaCPF = "";
    }
    ...
    $body = [
        ...
        "listaTransferencias" => [[
            ...
            "cpfBeneficiario" => $pessoaCPF,
            "cnpjBeneficiario" => $pessoaCNPJ,
            "dataTransferencia" => intval(date_format($dataTransferencia, 'dmY'))
        ]]
    ];


    //validacao
    if (!$body["listaTransferencias"][0]["agenciaCredito"]) {
        $statusId = 0;
        $status = "Nenhuma agência encontrada";
    }

    if (!$body["listaTransferencias"][0]["contaCorrenteCredito"]) {
        $statusId = 0;
    ...
    if (!$body["listaTransferencias"][0]["cpfBeneficiario"] && !$body["listaTransferencias"][0]["cnpjBeneficiario"]) {
        $statusId = 0;
    ...
    $contas->insertTransacao($values, "TRC");
} else {
}
