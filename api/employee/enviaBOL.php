...
if ($objData != NULL) {
    ...
    $body = [
        ...
        "lancamentos" => [[
            ...
            "documentoPagador" => intval($informacoesBancarias[0]["cpf"]),
            "codigoTipoPagador" => $codigoTipoPagador,
            "documentoBeneficiario" => intval($conta->{"pessoaCPF"}),
            "codigoTipoBeneficiario" => $codigoTipoBeneficiario,
            "dataPagamento" => intval(date_format($dataTransferencia, 'dmY'))
        ]]
    ];

    //validacao
    if (!$body["lancamentos"][0]["documentoBeneficiario"]) {
        $statusId = 0;
    ...
    $contas->insertTransacao($values, "BOL");
} else {
}
